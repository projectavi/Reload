...
# Code adapted from the https://github.com/if-loops/selective-synaptic-dampening repository

def SSD(data_loaders, model, criterion, optimizer, epoch, args, mask=None, wandb_run=None):
    forget_loader = data_loaders["forget"]
    retain_loader = data_loaders["retain"]
    full_loader = data_loaders["full"]
    ...
    # As dictated in the paper for ResNet18 and CIFAR tasks
    lambda_param = 1
    alpha_param = 10
    ...
    parameters = {
        ...
        "selection_weighting": alpha_param,  # Alpha from paper
    }

    optimizer = torch.optim.SGD(model.parameters(), lr=0.1)

    pdr = ParameterPerturber(model, optimizer, args.device, parameters=parameters)

    model = model.eval()

    # Calculate forget set importances
    sample_importances = pdr.calc_importance(forget_loader)

    # Calculate full dataset importances
    original_importances = pdr.calc_importance(full_loader)
    ...
    for i, (image, target) in enumerate(forget_loader):
        ...
        f_loss.update(loss.item(), image.size(0))
        f_accuracy.update(prec1.item(), image.size(0))
    ...
    for i, (image, target) in enumerate(retain_loader):
        ...
        loss = criterion(output, target)
        prec1 = utils.accuracy(output.data, target)[0]
        r_loss.update(loss.item(), image.size(0))
        r_accuracy.update(prec1.item(), image.size(0))
